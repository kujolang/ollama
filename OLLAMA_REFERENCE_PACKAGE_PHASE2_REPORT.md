...
`scripts/verify_installed_package.sh` creates a clean project, adds the immutable Ollama tag, installs twice, validates, sets `KUJO_MODULE_PATH` only to the two Kennel-installed package roots, and runs `tests/installed_consumer_smoke.kujo`. Result: `1/1` passed. The smoke verified package-root `ollama` and `provider` exports, transitive AI SDK `src.ai_sdk` resolution, local client configuration, driver identity, and NDJSON parsing.
...
`PROVIDER_PACKAGE_PATTERN_DRAFT.md` exists and reflects the final Ollama reality. Clean installation added one important convention: package-root shims must explicitly export imported symbols, and installed consumers must add Kennel package roots to `KUJO_MODULE_PATH` under the current Kujo runtime model. This is an implementation-era convention, not a claim that Provider Package Contract v1 is final.
